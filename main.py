...
def test_wall_blocking_right_up_up(x, y):
    for wall in global_list_of_walls:
        if classes.Wall(classes.Square(x, y+1, 1), classes.Square(x+1, y-1, 1), 1).equals(wall):
            return False
        if classes.Wall(classes.Square(x-1, y, 1), classes.Square(x+1, y+1, 1)).equals(wall):
            return False
        if classes.Wall(classes.Square(x+1, y, 1), classes.Square(x-1, y+1, 1)).equals(wall):
            return False
        if classes.Wall(classes.Square(x+1, y, 1), classes.Square(x, y+2, 1)).equals(wall):
            return False
        if classes.Wall(classes.Square(x+2, y, 1), classes.Square(x, y+1, 1)).equals(wall):
            return False
        if classes.Wall(classes.Square(x, y+1, 1), classes.Square(x+2, y+2, 1)).equals(wall):
            return False
        if classes.Wall(classes.Square(x+1, y+1, 1), classes.Square(x-1, y+2, 1)).equals(wall):
            return False
        if classes.Wall(classes.Square(x+1,y+1, 1), classes.Square(x, y+3, 1)).equals(wall):
            return False
        if classes.Wall(classes.Square(x+2, y+1, 1), classes.Square(x, y+2, 1)).equals(wall):
            return False
    return True
...
def test():
    create_board()
    place_tower(board, 0, 0)
    place_tower(board, 2, 1)
    place_Wall(board, 2, 1)
    switch_player()

    place_tower(board, 0, 1)
    place_tower(board, 2, 0)
    place_Wall(board, 2, 0)
    print_board(board)

    for x in range(len(global_list_of_walls)):
        print(str(global_list_of_walls[x].square1.x_coordinates) + "," + \
            str(global_list_of_walls[x].square1.y_coordinates) + " --- " + \
            str(global_list_of_walls[x].square2.x_coordinates) + "," + \
            str(global_list_of_walls[x].square2.y_coordinates) + " : " + \
            str(global_list_of_walls[x].player_occupying))
